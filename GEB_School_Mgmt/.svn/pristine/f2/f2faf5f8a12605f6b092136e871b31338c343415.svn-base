...
namespace GEB_School.Accounting
{
    public partial class RptPnL : PageBase
    {
        ...
        protected void btnExportExcel_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                string strPath = Request.Url.GetLeftPart(UriPartial.Authority) + "/Images/GEB LOGO school logo in english.jpg";
        ...
        protected void dlExpenseTransaction_OnItemDataBound(object sender, DataListItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (drv != null)
                {
                    ...
                    if (id == "0")
                    {
                        tdAccountName.Attributes.Add("style", "text-align:right;border:1px solid black;height:25px;");
                    }
                    else
                    {
