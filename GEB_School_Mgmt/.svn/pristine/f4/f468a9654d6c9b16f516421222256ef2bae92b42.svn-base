    ...
    public partial class GeneratePaySlip : PageBase
    {
        ...
        public void CalculatePayslip()
        {
            ...
            if (objResultPayslip.resultDT.Rows.Count > 0)
            {
                ...
                {
                    BindLeaveBalance(Convert.ToInt32(ViewState["EmployeeMID"].ToString()), ddlMonth.SelectedValue);
        ...
        {
            if (Convert.ToDouble(txtPayAbsenceDay.Text) > 0)
            {
                btnSendForApproval.Enabled = false;
            }
            else if (Convert.ToDouble(txtPayAbsenceDay.Text) == 0)
            {
                btnSendForApproval.Enabled = true;
            }
        }
                    ...
                    if (txtPayAbsenceDay.Text == txtTotal.Text)
                    {
                        ...
                        if (Page.IsValid == true)
                        {
                            ...
                            else
                            {
                                ...
                                objResults =
                                    objUserPayItemTemplateBl.EmployeeTemplate_SelectEarnDays(
                                        Convert.ToInt32(ViewState["EmployeeMID"].ToString()));

                                if (objResults.resultDT.Rows.Count > 0)
                                {
                                    if (ddlMonth.SelectedIndex ==
                                        Convert.ToInt32(objResults.resultDT.Rows[0]["Month"].ToString()) &&
                                        ddlYear.SelectedValue == objResults.resultDT.Rows[0]["Year"].ToString())
                                    {
                                ...
                                ScriptManager.RegisterStartupScript(this, GetType(), "alert",
                                    "alert('Payslip is generated Successfully.');", true);
                        ...
                        if (GDBalance.Rows[j].Cells[1].Text == "1")
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    GDBalance.Rows[j].Cells[5].Text =
                                        (AvailDays - Convert.ToDouble(txt.Text)).ToString();
                                }
                            }
                        }
                    }
        ...
        private void BindLeaveBalance(int intEmployeeMID, string strMonth)
        {
            try
            {
                ...
                objResult =
                    objLeaveBl.Leave_Select_ForPayrollBalance(intEmployeeMID, strMonth);
                if (objResult != null)
                {
                    if (objResult.resultDT.Rows.Count > 0)
                    {
                ...
                for (int i = 0; i < GDBalance.Rows.Count; i++)
                {
                    ...
                    for (int k = 0; k < objResultLeave.resultDT.Rows.Count; k++)
                    {
                        if (GDBalance.Rows[i].Cells[0].Text == (objResultLeave.resultDT.Rows[k]["LeaveID"].ToString()))
                        {
                            ...
                            double dbDiff = 0.0;
                            dbBalance = Convert.ToDouble(GDBalance.Rows[i].Cells[3].Text.ToString());
                            if ((objResultLeave.resultDT.Rows[k]["HalfDay"].ToString()) == "1")
                            {
                                dbHalfDay = 0.5;
                            }
                            dbTotal = dbFullDay + dbHalfDay;
                            dbDiff = dbBalance - dbTotal;
